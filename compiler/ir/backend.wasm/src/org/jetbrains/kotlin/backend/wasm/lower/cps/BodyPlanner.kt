    ...
    fun plan(): BodyPlan? {
        ...
        return try {
            ...
            if (last.terminator == null) {
                last.terminator = Terminator.Ret(cpsDefaultValue(func.returnType))
            }
            val plan = BodyPlan(blocks.first(), blocks, locals)
                ...
                if (containsTargetCall(value)) {
                    ...
                    when (peeled) {
                        ...
                        is IrWhen -> {
                            ...
                            if (end.terminator == null) end.terminator = Terminator.Ret(cpsDefaultValue(func.returnType))
                            return end
                        }
                        is IrBlock -> {
